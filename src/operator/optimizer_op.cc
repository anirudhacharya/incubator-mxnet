...
DMLC_REGISTER_PARAMETER(AMSGradParam);
...
NNVM_REGISTER_OP(amsgrad_update)
MXNET_ADD_SPARSE_OP_ALIAS(amsgrad_update)
.describe(R"code(Update function for AMSGrad optimizer.

AMSGrad update consists of the following steps, where g represents gradient and m, v
are 1st and 2nd order moment estimates (mean and variance).
...
.set_num_inputs(3)
.set_num_outputs(1)
.set_attr_parser(ParamParser<AMSGradParam>)
.set_attr<nnvm::FInferShape>("FInferShape", ElemwiseShape<3, 1>)
.set_attr<FResourceRequest>("FResourceRequest",
  [](const NodeAttrs& attrs) {
    return std::vector<ResourceRequest>{ResourceRequest::kTempSpace};
...
.set_attr<FCompute>("FCompute<cpu>", AMSGradUpdate<cpu>)
.set_attr<FComputeEx>("FComputeEx<cpu>", AMSGradUpdateEx<cpu>)
.add_argument("weight", "NDArray-or-Symbol", "Weight")
.add_argument("grad", "NDArray-or-Symbol", "Gradient")
.add_argument("mean", "NDArray-or-Symbol", "Moving mean")
.add_argument("var", "NDArray-or-Symbol", "Moving variance")
.add_arguments(AMSGradParam::__FIELDS__());
