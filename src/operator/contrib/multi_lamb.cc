...
NNVM_REGISTER_OP(multi_lamb)
.describe(R"code(Compute the LAMB coefficients of multiple weights and grads from their sums of square"
)code" ADD_FILELINE)
.set_num_inputs(4)
.set_num_outputs(1)
...
.set_attr<FCompute>("FCompute<cpu>", MultiLAMB<cpu>)
.add_argument("lrs", "NDArray-or-Symbol", "Learning rates to scale by LAMB coefficient")
